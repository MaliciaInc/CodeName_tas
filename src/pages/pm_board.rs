use iced::{
    widget::{
        button, container, mouse_area, scrollable, text, text_editor, text_input, Column, Row,
        Space,
    },
    Alignment, Color, Element, Length, Padding, Theme, Vector,
};
...
pub fn pm_board<'a>(
    ...
    let header = Row::new()
        ...
        .push(ui::primary_button(
            ...
            Message::Pm(PmMessage::OpenGlobalCreate),
        ))
        .padding(Padding {
            top: 0.0,
            right: 0.0,
    ...
    // ✅ OPTIMIZED: Iterar solo columnas, cards por lookup O(1) + collect O(k)
    for col in &board_data.columns {
        let cards = board_data.get_column_cards(&col.id);
    ...
    // FIX VISUAL: Mouse Area debe envolver el contenido
...
fn render_column<'a>(
    ...
    // ✅ Hot-path: internar col.id -> PmId una vez
    let col_id = state.pm_ids.get(col.id.as_str());
    let col_id_for_style = col_id.clone();

    // ✅ Hot-path: para cada card, internar card.id -> PmId una vez por iteración
    for card in cards {
        let pm_card_id = state.pm_ids.get(card.id.as_str());

        let is_dragging_active = match &state.pm_state {
            PmState::Dragging { card_id, active, .. } => {
                *active && card_id.as_ref() == pm_card_id.as_ref()
            }
            _ => false,
        };

        // ✅ Hover de card sin allocs (PmId vs PmId)
        let is_hovered = match &state.hovered_card {
            Some(h) => h.as_ref() == pm_card_id.as_ref(),
            None => false,
        };

        if !is_dragging_active {
            // Movemos el PmId al render_card (sin Strings por frame)
            cards_col = cards_col.push(render_card(t, *card, pm_card_id, 1.0, is_hovered));
        } else {
            ...
            cards_col = cards_col.push(
                container(Space::new())
                    ...
                    .style(move |_: &Theme| {
                        let mut s =
                            ui::container_style(ui::alpha(t.accent, 0.1), Color::TRANSPARENT);
    ...
    // Scroll vertical dentro de la columna
    let column_content = scrollable(cards_col)
        .direction(scrollable::Direction::Vertical(
            scrollable::Scrollbar::new().width(4.0),
        ))
        .height(Length::Fill);

    let body = Column::new().spacing(12).push(header).push(column_content);
    ...
    let content = container(body)
        ...
        .style(move |_: &Theme| {
            ...
            // ✅ PmId vs PmId (sin allocs)
            if let Some(hovered) = &state.hovered_column {
                if hovered.as_ref() == col_id_for_style.as_ref() {
    ...
    mouse_area(content)
        .on_enter(Message::Pm(PmMessage::ColumnHovered(col_id)))
        .into()
}
...
fn render_card(
    t: ui::Tokens,
    card: &Card,
    card_id: crate::app::PmId,
    alpha_mul: f32,
    is_hovered: bool,
) -> Element<'_, Message> {
    let title_color = ui::alpha(t.foreground, alpha_mul);
    let bg_color = if alpha_mul < 1.0 {
        ui::alpha(t.card, 0.2)
    } else {
        t.card
    };
    ...
    let priority_color = match card.priority.as_str() {
        "High" => Color::from_rgb8(239, 68, 68),
        "Low" => Color::from_rgb8(34, 197, 94),
        _ => Color::from_rgb8(234, 179, 8),
    };
    ...
    let final_content = if has_desc {
        content.push(
            text(&card.description)
                .size(12)
                .color(ui::alpha(t.muted_fg, alpha_mul * 0.8))
    ...
    let card_box = container(final_content)
        .padding(12)
        .width(Length::Fill)
        .style(move |_: &Theme| {
            ...
            // ✅ Hover highlight sin Strings, sin allocs
            if hovered {
                s.background = Some(ui::alpha(t.accent, 0.05).into());
                s.border.color = ui::alpha(t.accent, 0.45);
            }
    ...
    // ✅ clones baratos (Arc bump). No hay allocs de String por frame.
    mouse_area(card_box)
        .on_press(Message::Pm(PmMessage::DragStart(card_id.clone())))
        .on_enter(Message::Pm(PmMessage::CardHovered(card_id)))
        .into()
}

// render_modal se mantiene igual (UI de crear/editar)
pub fn render_modal<'a>(
    t: ui::Tokens,
    title: &'a str,
    desc: &'a text_editor::Content,
    priority: &'a str,
    is_new: bool,
) -> Element<'a, Message> {
    ...
    let p_btn = |label: &str, val: &str| {
        ...
        let mut btn = button(
            text(label.to_string()).size(12).style(move |_| iced::widget::text::Style {
                color: Some(if is_selected { t.background } else { t.muted_fg }),
            }),
        )
            .padding([6, 12])
    ...
    let priority_row = Row::new()
        .spacing(8)
        .align_y(Alignment::Center)
        .push(text("Priority:").size(12).style(move |_| iced::widget::text::Style {
            color: Some(t.muted_fg),
        }))
        .push(p_btn("Low", "Low"))
    ...
    let form = Column::new()
        .spacing(16)
        .push(text(header_text).size(20).style(move |_| iced::widget::text::Style {
            color: Some(t.foreground),
        }))
        .push(
            Column::new()
                .spacing(6)
                .push(text("Title").size(12).style(move |_| iced::widget::text::Style {
                    color: Some(t.muted_fg),
                }))
                .push(
                    text_input("", title)
        ...
        .push(
            Column::new()
                .spacing(6)
                .push(text("Description").size(12).style(move |_| iced::widget::text::Style {
                    color: Some(t.muted_fg),
                }))
                .push(
                    text_editor(desc)
        ...
        .push(
            Row::new()
                ...
                .push(ui::primary_button(
                    t,
                    "Save Task".to_string(),
                    Message::Pm(PmMessage::Save),
                ))
                .push(ui::ghost_button(
                    t,
                    "Cancel".to_string(),
                    Message::Pm(PmMessage::Cancel),
                ))
                .push(Space::new().width(Length::Fill))
                .push(if !is_new {
                    ui::danger_button(t, "Delete".to_string(), Message::Pm(PmMessage::Delete))
                } else {
                    Space::new().into()
                }),
        );
